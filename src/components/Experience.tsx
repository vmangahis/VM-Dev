  ...
  useEffect(() => {
    const formatDate = (experienceParam: ExperiencesObject[]): void => {
      experienceParam.forEach((val) => {
        ...
        let smonth = start_date.toLocaleDateString('default', {
          month: 'long',
        });
        ...
        if (val.endDate != null) {
          ...
          let end_month = end_date.toLocaleDateString('default', {
            month: 'long',
          });
      ...
      <div className="experience-timeline">
        ...
        {experiences.map((val, index) => {
          return (
            <TimelineCard
              key={val.roleName}
              expIcon={<FontAwesomeIcon icon={faBusinessTime} inverse />}
              rowType={index % 2 != 0 ? 'even' : 'odd'}
              title={val.roleName}
              company={val.company}
              st_date={val.st_date_format}
              ed_date={val.ed_date_format}
            />
          );
        })}
      </div>
